...
if [ -n "$1" ]; then
    ...
    display_clipboard="$clipboard_content"
    if [ ${#display_clipboard} -gt 50 ]; then
        display_clipboard="${display_clipboard:0:47}.."
    fi

    if [ -n "$clipboard_content" ]; then
        choice=$(dialog --stdout --title "URL Input" --menu "Choose input method:" 12 70 2 \
            1 "Use clipboard: $display_clipboard" \
            2 "Enter manually")
    else
        choice=2
    fi
    ...
    case $choice in
        1)
            ...
            ;; 
        2)
            manual_input=$(dialog --stdout --title "URL Input" --inputbox "Enter a URL or Username (or leave blank to use clipboard):" 8 70)
...
if [[ ! "$user_input" =~ "://" && ! "$user_input" =~ "/" ]]; then
    ...
    case $platform_choice in
        ...
        2) 
            ...
            FOUND_FILE="${TMP_DIR}/found_platforms.txt"
            > "$LOG_FILE"
            > "$FOUND_FILE"
            ...
            for platform_info in "${platforms_to_check[@]}"; do
                ...
                http_code=$(curl -s -L -o /dev/null -w "%{http_code}" "$url_to_check")

                echo "[$platform_name] Checking: $url_to_check - Result: HTTP $http_code" >> "$LOG_FILE"

                if [[ "$http_code" == "200" ]]; then
                    echo "$platform_name" >> "$FOUND_FILE"
                    echo "$url_to_check" >> "$FOUND_FILE"
                fi
                checked_count=$((checked_count + 1))
                percentage=$((checked_count * 100 / total_to_check))
                echo $percentage
                echo "XXX"
                echo "Checking $platform_name..."
                echo "XXX"
            done
            ...
            is_first_found="on"
            while read -r platform_name && read -r url; do
                found_platforms_options+=("$platform_name" "$url" "$is_first_found")
                is_first_found="off"
            done < "$FOUND_FILE"
            rm "$FOUND_FILE"
